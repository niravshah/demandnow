...
public class RegistrationIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            ...
            sharedPreferences.edit().putBoolean(GDNSharedPrefrences.SENT_TOKEN_TO_SERVER, true).apply();
            // [END register_for_gcm]
        } catch (Exception e) {
            Log.d(TAG, "Failed to complete token refresh", e);
            // If an exception happens while fetching the new token or updating our registration data
            // on a third-party server, this ensures that we'll attempt the update at a later time.
            sharedPreferences.edit().putBoolean(GDNSharedPrefrences.SENT_TOKEN_TO_SERVER, false).apply();
        }
        // Notify UI that registration has completed, so the progress indicator can be hidden.
        Intent registrationComplete = new Intent(GDNSharedPrefrences.REGISTRATION_COMPLETE);
